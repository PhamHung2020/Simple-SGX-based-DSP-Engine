...
def load_secure_sgx_data_for_cross(is_split=True, is_random=False, train_size=0.8, val_size=0.1):
    data_files = {}
    for operator in SECURE_SGX_MAPPING_FOR_CROSS:
        data_files[operator] = {
            'files': [],
            'label': SECURE_SGX_MAPPING_FOR_CROSS[operator]['label']
        }

        for dir in SECURE_SGX_MAPPING_FOR_CROSS[operator]['dirs']:
            files = [f for f in os.listdir(dir) if f.endswith('.csv')]
            data_files[operator]['files'].extend([os.path.join(dir, file) for file in files])
